...
var ENV = require("system").env,
    ...
    OS = require("os"),
    projectName = "CPDoubleBindingsError";
...
app (projectName, function(task)
{
    ...
    task.setBuildIntermediatesPath(FILE.join("Build", "CPDoubleBindingsError.build", configuration));
    task.setBuildPath(FILE.join("Build", configuration));

    task.setProductName("CPDoubleBindingsError");
    task.setIdentifier("net.Krodelin.CPDoubleBindingsError");
    task.setVersion("1.0");
    task.setAuthor("Krodelin Software");
    task.setEmail("info@Krodelin.net");
    task.setSummary("CPDoubleBindingsError");
...
task ("desktop", ["release"], function()
{
    ...
    require("cappuccino/nativehost").buildNativeHost(FILE.join("Build", "Release", projectName), FILE.join("Build", "Desktop", projectName, "CPDoubleBindingsError.app"));
    printResults("Desktop")
});

task ("run-desktop", ["desktop"], function()
{
    OS.system([FILE.join("Build", "Desktop", projectName, "CPDoubleBindingsError.app", "Contents", "MacOS", "NativeHost"), "-i"]);
});
